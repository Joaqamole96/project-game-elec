...
public class BSPGenerator
{
    ...
    private void SplitPartition(PartitionModel partition, PartitionConfig config, System.Random random)
    {
        ...
        if (partition.Bounds.width <= config.MaxSize && 
            partition.Bounds.height <= config.MaxSize)
            return;

        // If too small, don't split
        if (partition.Bounds.width <= config.MinSize || 
            partition.Bounds.height <= config.MinSize)
            return;
